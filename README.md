# Devskiller programming task sample - R
...
Your project tests will be executed with following command:
...
```json
{
  "verification" : {
    ...
    "pathPatterns" : ["**/test-verification-*.R"]
  }
}
```
...
In our sample project, all verification test file names use the `test-verification-calculator.R` pattern, e.g. `test-verification-calculator.R`.
In this case the following configuration will be sufficient:
